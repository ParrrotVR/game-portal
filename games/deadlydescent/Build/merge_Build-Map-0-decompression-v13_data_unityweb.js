...
// File merging script for deadlydescent
...
// Initialize merging for deadlydescent
(async () => {
  try {
    // Update loading text
    ...
    // Merge Build-Map-0-decompression-v13.data.unityweb
    const fileParts = getParts("Build-Map-0-decompression-v13.data.unityweb", 1, 4);
    const mergedUrl = await mergeFiles(
      fileParts, 
      "Build-Map-0-decompression-v13.data.unityweb", 
      (progress, filename) => {
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...rest) {
      if (url.includes("Build-Map-0-decompression-v13.data.unityweb")) {
        url = mergedUrl;
      }
