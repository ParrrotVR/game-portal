...
(async () => {
  try {
    ...
    // Merge pvzert.data.unityweb.part1
    const fileParts = getParts("pvzert.data.unityweb_parts/pvzert.data.unityweb.part1", 1, 3);
    const mergedUrl = await mergeFiles(
      fileParts, 
      "pvzert.data.unityweb.part1", 
      (progress, filename) => {
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...rest) {
      if (url.includes("pvzert.data.unityweb.part1")) {
        url = mergedUrl;
      }
